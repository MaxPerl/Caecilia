...
for(s2=s;s2&&s2.time==s.time;s2=s2.ts_next){switch(s2.type){case C.KEY:if(!s.fmt.keywarn||s2.invis||(!s2.k_sf&&!s2.k_a_acc))
continue
...
function set_scale(s){var new_dy,st=staff_tb[s.st].staffscale==1?-1:s.st,new_scale=s.p_v.scale
if(new_scale==1){if(stv_g.st!=st)
set_sscale(st)
return}
new_dy=posy
...
abc2svg.version="v1.22.16";abc2svg.vdate="2024-05-27"
